...
\end{figure}
\noindent
As we shall see in section \ref{learningAlgorithm}, it's often useful to compute the output of $h$ not of a single training sample $\mathbf{x}_i$, but of a \textbf{batch} of $m$ examples $\mathbf{X} = \begin{bmatrix} \mathbf{x}_1 & \dots & \mathbf{x}_m \end{bmatrix}^T$, such that each $\mathbf{x}_1$ to $\mathbf{x}_m$ is a row of the matrix $\mathbf{X}$. We can of course equally well arrange the vectors $\tilde{\mathbf{x}}_i$ of inputs with added bias components in a matrix to get:
$$
\tilde{\mathbf{X}} = \begin{bmatrix}
	\tilde{\mathbf{x}}_1 & \dots & \tilde{\mathbf{x}}_m
\end{bmatrix}^T
$$

In this batch processing scheme, the activation at the first layer is an entire matrix $\mathbf{A}_1 = \mathbf{X}\mathbf{W}_1$, such that row $i$ of $\mathbf{A}_1$ is identical to the activation vector $\mathbf{a}_1$ for sample $\mathbf{x}_i$ in the batch. Similarly, $\mathbf{Z}_1$ is obtained by applying the activation function $\sigma$ element wise to $\mathbf{A}_1$, such that $(\sigma(\mathbf{A}_1))_{(i,j)} = (\mathbf{Z}_1)_{(i,j)}$. In general, the output of layer $l$ is $\mathbf{Z}_l = \sigma(\mathbf{Z}_{l-1}\mathbf{W}_l)$.
\\\\
