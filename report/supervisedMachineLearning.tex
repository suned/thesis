...
In order to get an unbiased empirical estimate of $E$, we can split $\mathcal{D}$ into two datasets: $\dval$ containing $K$ samples, and $\dtrain$ containing $N - V$ samples. $\dtrain$ is used by the learning system to find a function $g^- \in \mathcal{H}$. The minus indicates that the function was selected using only a subset of $\mathcal{D}$. $\dval$ is used to compute $\valerror{g^-}$ as an unbiased estimate of $E$.
\\\\
...
$$
To understand how $E_{cv}$ relates to $E$. We want to know $E$, but it would be almost as useful to know the expected $E$ of our learning system, when trained on any dataset $\mathcal{D}$ of size $N$. For this purpose, we can define 
$$
\tilde{E}(N) = \mathbb{E}_{\mathcal{D}}[E(g)]
$$
That is, the expected generalisation error with respect to datasets of size $N$. The expected value of $E_{cv}$ is $\tilde{E}(N - \frac{N}{K})$